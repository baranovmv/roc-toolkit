...
sndio::DeviceState Receiver::get_state() {
    roc_panic_if(init_status_ != status::StatusOK);

    return pipeline_.source().state();
}
...
status::StatusCode Receiver::poll(unsigned state_mask, core::nanoseconds_t deadline) {
    roc_panic_if(init_status_ != status::StatusOK);

    return pipeline_.source().poll(state_mask, deadline);
}
